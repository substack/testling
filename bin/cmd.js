    ...
    if (argv.bcmd || argv.x) {
        ...
        var browser = launch
            ...
            && (/linux|bsd/i.test(process.platform) // xvfb-run
                ...
                : launch.browsers.local.find(function (b) { return b.headless; })
            );
...
function getHTML (cb) {
    ...
    if (pkg.testling.html) {
        fs.readFile(path.join(dir, pkg.testling.html), function (err, src) {
            if (err) {
                console.error('while loading testling.html: ' + err);
            } else {
